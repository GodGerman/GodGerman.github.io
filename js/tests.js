...
export function runTests() {
  ...
  const subnets = computeSubnets(cidr.network, 16, 256); // 148.204.0.0/16 -> 256 subnets
  // We can't easily test massive output here without clogging but we can test properties.
  // Test a smaller case: /24 -> 4 subnets
  ...
  const subnetsInvalid = computeSubnets([192, 168, 0, 0], 24, 0);
  ...
  results.push(assertEqual("Subnets no potencia de 2", Boolean(subnetsNotPower.error), true));
